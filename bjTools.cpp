...
void getVersionStringFromNumber (const char* verStringIn, char* verStringOut)
{
	...
	if (ver[2]==0 && ver[3]==0)
		sprintf (verStringOut, "v%d.%d", ver[0], ver[1]);
	else
	{
		if (ver[3]==0)	sprintf (verStringOut, "v%d.%d%d", ver[0], ver[1], ver[2]);
		else			sprintf (verStringOut, "v%d.%d%d%d", ver[0], ver[1], ver[2], ver[3]);
	}
}
